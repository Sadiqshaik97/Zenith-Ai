...
function getGeminiApiKey() {
  const localKey = localStorage.getItem('zenith_gemini_api_key');
...
async function processAiActions(rawText: string): Promise<string> {
  let jsonString = '';
      ...
      try {
        ...
        for (const [i, t] of tasksToAdd.entries()) {
          if (t.title) {
            const tempTask = {
              title: t.title,
              description: t.description || `Auto-generated by Aura AI`,
...
When scheduling or planning the week, you MUST first print out a beautiful day-by-day markdown schedule table or list proposing the allocation of the user's tasks to the weekdays, and then append the RUN_SCHEDULER action. Do not just say you are running the scheduler — print the exact schedule data first so it looks like a real AI assistant planner. Be encouraging, concise, and professional. Do not use markdown inside the action block.`;
        ...
        const rawHistory = filterAlternateHistory(
        ...
        const runOfflineFallback = async (rateLimited = false) => {
          const browserApiKey = getGeminiApiKey();
          if (browserApiKey && browserApiKey.trim() !== '' && !rateLimited) {
            try {
              ...
              const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${browserApiKey}`, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json'
                },
              ...
              if (response.ok) {
                ...
                if (rawText) {
                  replyText = await processAiActions(rawText);
                  return;
                }
              }
            } catch (err) {
              console.error('Error during direct client-side Gemini API call:', err);
            }
          }

          // Legacy offline heuristics fallback
          ...
          if (cleanText.includes('week tasks') || cleanText.includes('tasks for the week') || cleanText.includes('plan my week tasks')) {
            ...
            const incompleteTasks = [...useStore.getState().tasks]
              .filter((t) => t.status !== 'Done')
              .sort((a, b) => (b.priorityScore || 0) - (a.priorityScore || 0));
        ...
        try {
          ...
          const data = await apiCall('/api/chat', 'POST', { messages: messagesPayload });
